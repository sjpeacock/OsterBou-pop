###############################################################################
#
# Code to plot conceptual and supplemental figures 
#
...
# Color for ranges
colRange <- c(wes_palette("Darjeeling1")[c(5, 2, 3, 1)], "#000000")
names(colRange) <- c("winter",  "spring", "calving", "summer", "fall")
...
quartz(width = 4.5, height = 2, pointsize = 10)
par(mfrow = c(1,1), mar = c(3,4,1,8))

plot(z, DOY, "n", ylim = c(0,1), ylab  = "Proportion arresting", las = 1, xlab = "", xaxs = "i", bty = "l")
abline(h = 1, col = scenCols[1], lwd = 2)
abline(h = 0.5, col = scenCols[2], lwd = 1.2)
abline(h = 0, col = scenCols[3], lwd = 2)
lines(z, pI4, col = scenCols[4], lwd = 1.2)
legend(5880, 1, lwd = c(2, 1.2, 2, 1.2), col = scenCols,  c("100%", "50%", "0%", "Variable"), xpd = NA, bty = "n", cex = 0.8, title = "Arresting")
...
for(r in 1:2){
	lines(xDate, ccAnomalies[, r], col = ccCol[r], lwd = 0.8)
	lines(xDate, ccAnomalies[, r+2], col = ccCol[r], lwd = 1.5)
}
...
for(r in 1:3){
	lines(xDate, allTemps[[r]][, x.ind], col = c(1, ccCol)[r], lwd = 1.5)
}
